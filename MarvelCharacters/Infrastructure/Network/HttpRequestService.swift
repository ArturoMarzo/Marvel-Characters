...
/*
 ...
 The UsersServices protocol has been developed here and not in a extension because
 in tests the methods couldn't be overriden
 */
class HTTPRequestService {
    static let genericErrorCode = 999 // generic error code for unexpected failures retrieving information
    
    // Method to create a generic error to return if something unexpected happened
    static func genericError() -> Error {
        return NSError(domain: NSURLErrorDomain, code: HTTPRequestService.genericErrorCode, userInfo:nil)
    }
