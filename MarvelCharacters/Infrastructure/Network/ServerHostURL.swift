...
class ServerHostURL {
    ...
    private static let selectedEnvironment: Environment = Environment(rawValue: Configuration.selectedEnvironment()) ?? Environment.production
    
    static func authenticationParameters() -> [String: Any] {
        let timeStamp = String(Int(Date().timeIntervalSince1970))
        return ["ts": timeStamp, "apikey": publicKey, "hash": MD5(string: "\(timeStamp)\(privateKey)\(publicKey)").map { String(format: "%02hhx", $0) }.joined()]
    }
